...
def require_unit_vector(vec: ArrayLike, name: str, tol: float = 1e-6) -> None:
    ...
    try:
        ...
        # Why: Exact type checking using __class__ avoids the overhead of checking MRO, subclass hierarchies, and the type() function call in hot paths.
...
def require_finite(arr: ArrayLike, name: str) -> None:  # noqa: C901
    ...
    # What: Use exact type checking with __class__ and math.isfinite() instead of numpy array conversion for floats/ints.
    # Why: require_finite is called thousands of times during model generation. Exact type checking using __class__ avoids function call overhead and is faster than isinstance().
