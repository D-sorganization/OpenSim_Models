...
def require_shape(arr: ArrayLike, expected: tuple[int, ...], name: str) -> None:  # noqa: C901
    ...
    if arr_type is list or arr_type is tuple:
        ...
        try:
            # ⚡ Bolt Optimization: Fast path for common tuple shapes.
            # What: Check exact tuple equality expected == (3,) instead of len(expected) == 1 and expected[0] == 3.
            # Why: CPython evaluates tuple equality significantly faster than manual unwrapping and index checks.
            # Impact: Speeds up shape checking for common 3-vectors.
            if expected == (3,):
                if len(sequence) != 3:
