...
**Learning:** Running postcondition validations (like coordinate bounds checking) on the in-memory `ET.Element` tree _after_ executing serialization via `serialize_model(root)` (which uses `ET.tostring`) results in unnecessary stringification overhead when validation fails.
**Action:** Always run postcondition validations directly on the in-memory `ET.Element` tree _before_ executing serialization to fail fast and avoid redundant string generation overhead in error paths.
...
## 2026-10-05 - Avoid isinstance fallback in list/tuple fast paths
