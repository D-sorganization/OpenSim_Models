...
### Performance
- Optimized `ensure_coordinates_within_bounds` in `postconditions.py` by replacing `.find()` with direct child iteration, bypassing ElementPath regex overhead for shallow lookups.
