...
### Performance
- Optimized XML element finding in `ensure_coordinates_within_bounds`, `set_coordinate_default`, and `set_coordinate_defaults` by replacing `Element.find()` with direct child iteration to avoid `ElementPath` regex parsing overhead.
